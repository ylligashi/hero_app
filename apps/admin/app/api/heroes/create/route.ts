...
async function createOllamaModel(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Ollama API error: ${errorData.message || response.statusText}`,
      );
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, description, systemPrompt, modelName, avatarUrl } =
    ...
    try {
      ...
      await createOllamaModel(
        ...
        systemPrompt ||
          `You are ${name}, ${description || "a helpful AI assistant."}`,
      );
      ...
      return NextResponse.json(
        {
          ...hero,
          warning:
            "Hero created in database, but failed to create custom Ollama model",
        },
        { status: 201 },
      );
    }
